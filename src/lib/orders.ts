...
export async function createGuestOrder(input: CreateGuestOrderInput) {
  ...
  const deliveryFeeLkr = Number(input.deliveryFeeLkr || 0);
  const orderTotalLkr = subtotalLkr + deliveryFeeLkr;
  ...
  const { error: orderError } = await supabase.from("orders").insert({
    ...
    delivery_slot_start: input.deliverySlotStart || null,
    delivery_slot_end: input.deliverySlotEnd || null,
    delivery_status: "PENDING",

    delivery_distance_km: input.deliveryDistanceKm ?? null,
    delivery_vehicle_type: input.deliveryVehicleType || null,
    delivery_fee_lkr: deliveryFeeLkr,
    delivery_pricing_mode: input.deliveryPricingMode || null,
    order_total_lkr: orderTotalLkr,
